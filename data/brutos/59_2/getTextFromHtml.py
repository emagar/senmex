...
soup = BeautifulSoup(file, features = "lxml")             # passes its contents to beautiful soup
file.close()                           # shuts the file
text = soup.get_text().encode("utf-8") # gets text only without html markups
## text = soup.get_text().encode("latin1") # gets text only without html markups
##text = soup.get_text().encode("ISO-8859-1") # gets text only without html markups


f = open(newfilename, "wb")      # creates a new file to dump the text, binary
